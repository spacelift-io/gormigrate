...
func TestMigration(t *testing.T) {
	forEachDatabase(t, func(db *gorm.DB) {
		...
		assert.EqualValues(t, 2, tableCount(t, db, "migrations"))

		err = m.RollbackLast()
		assert.NoError(t, err)
		assert.True(t, db.HasTable(&Person{}))
		assert.False(t, db.HasTable(&Pet{}))
		assert.EqualValues(t, 1, tableCount(t, db, "migrations"))

		err = m.RollbackLast()
		assert.NoError(t, err)
		assert.False(t, db.HasTable(&Person{}))
		assert.False(t, db.HasTable(&Pet{}))
		assert.EqualValues(t, 0, tableCount(t, db, "migrations"))
	})
}
...
func TestRollbackTo(t *testing.T) {
	forEachDatabase(t, func(db *gorm.DB) {
		...
		assert.EqualValues(t, 3, tableCount(t, db, "migrations"))
